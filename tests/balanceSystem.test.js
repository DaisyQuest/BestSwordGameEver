...
describe("balanceSystem", () => {
  ...
  it("applies balance penalties, posture drag, and impact stress", () => {
    const world = createPhysicsWorld({ gravity: { x: 0, y: 0, z: 0 }, maxSpeed: 100 });
    world.createBody({ id: "hero", velocity: { x: 10, y: 0, z: 4 }, damping: 1 });
    ...
    expect(report.target).toBeCloseTo(0.435, 3);
    expect(report.current).toBeCloseTo(0.435, 3);
    expect(report.posture).toBe("stumbling");
    expect(report.limbPenalty).toBeCloseTo(0.35);
    expect(report.speedPenalty).toBeCloseTo(0.215, 3);
    ...
    expect(body.velocity.z).toBeCloseTo(2);
  });

  it("handles validation errors and low impact updates", () => {
    const world = createPhysicsWorld({ gravity: { x: 0, y: 0, z: 0 } });
    world.createBody({ id: "hero", velocity: { x: 1, y: 0, z: 0.5 }, damping: 1 });
    const model = createPlayerModel({
    ...
    expect(() =>
      balanceSystem.applyBalance(world, "hero", model, {
        deltaMs: 16,
        previousVelocity: { x: 0, y: 0, z: "nope" }
      })
    ).toThrow(RangeError);
    expect(() =>
      balanceSystem.applyBalance(world, "hero", model, {
  ...
  it("applies fall drag and skips impact stress below threshold", () => {
    const world = createPhysicsWorld({ gravity: { x: 0, y: 0, z: 0 } });
    world.createBody({ id: "hero", velocity: { x: 2, y: 0, z: 1 }, damping: 1 });
    ...
    expect(world.getBody("hero").velocity.z).toBeCloseTo(0.1);
  });

  it("keeps steady posture when balance is high", () => {
    const world = createPhysicsWorld({ gravity: { x: 0, y: 0, z: 0 } });
    world.createBody({ id: "hero", velocity: { x: 0.1, y: 0, z: 0 }, damping: 1 });
    const model = createPlayerModel({
